...
const BotChoices = ({className}) => {
    ...
    const choices = [
        ...
        },
        {
            ...
            ans: [
                <a href={`mailto:abdulrahman.saad2303@gmail.com`}>Click to Send Message.</a>,
                <a href='https://www.linkedin.com/in/abdulrahman-saad/'>Or check my LinkedIn.</a>,
  ...
  return (
    <div className={className}>
        {choices.map((choice, i) => (
            <button key={`${choice} ${i}`} onClick={() => handleChoice(i)} className={`border border-red-dev rounded-3xl py-3 px-4 w-fit mb-2 hover:shadow-xl hover:-translate-y-1 transition duration-400 cursor-pointer choice opacity-0 translate-y-5`}>{choice.q}</button>
        ))}
        {answers && answers.map((answer, i) => {
            if (i === answers.length - 1) {
                return (<><Message className={`msg opacity-0 translate-y-5`}>{answer}</Message>
                <BotChoices/></>)
