...
const ProjectCard = ({ project, index }) => {
  ...
  return (
    <>
      {/* Image Section */}
      <div className="flex-1 w-full group relative overflow-hidden rounded-2xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-400 dark:border-zinc-200">
        <img
          src={image}
          alt={title}
        ...
        <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center gap-6">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="text-white hover:text-red-light transition-colors p-3 bg-black/20 backdrop-blur-md rounded-full"
          >
            <FaGithub size={24} />
          </a>
          <a
      ...
      >
        ...
        >
          {tech.map((t) => (
            <span
              key={t}
              className="text-[11px] font-bold text-zinc-500 uppercase tracking-widest border-b border-zinc-300 dark:border-zinc-600 hover:border-red-light hover:text-red-light transition-colors duration-300 pb-1"
            >
              {t}
            </span>
          ))}
